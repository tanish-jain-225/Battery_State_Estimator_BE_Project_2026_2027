...
| **What is the role of EKF, Coulomb Counting, and the STM32 hardware in this project?** | EKF and Coulomb Counting are implemented strictly as **offline accuracy benchmarks/baselines** to evaluate the ESN's performance, rather than parts of the deployed pipeline. The STM32 microcontroller deployment is a **feasibility check** to prove that the ESN's reservoir-computing architecture (demonstrated via the classifier variant) can run under real low-power embedded constraints. Porting the SOC/SOH estimator itself to STM32 is future work. |
| **What are the limitations and validation scope of this system?** | The primary deliverable is a software-validated data-driven ESN estimator (for SOC/SOH), not a physical hardware product or a hybrid system. The system runs on a simulated equivalent cell abstraction. Success is defined by algorithmic accuracy and efficiency, not by the hardware demo itself. Full production deployment requires HIL testing, physical cell validation, and pack-level scaling. |
