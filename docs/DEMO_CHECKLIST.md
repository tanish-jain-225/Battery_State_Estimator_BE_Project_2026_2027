...
| **How does the Sage-Husa EKF differ from a standard EKF?** | The **Sage-Husa Adaptive EKF** dynamically estimates and updates the process noise covariance matrix $\mathbf{Q}$ and measurement noise covariance $R$ at each timestep based on measurement innovation. This prevents filter divergence and retains accuracy when sensor noise properties change or model parameters drift. |
