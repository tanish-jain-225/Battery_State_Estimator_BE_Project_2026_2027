...
class Config:
    ...
    # Increased from 200→500: more priming steps compensates for the higher
    # leak rates, letting the reservoir reach a deeper steady state.
    ...
    # Defaults to 300.0 seconds everywhere to match Gunicorn timeout and support 2500-sample training.
    ONLINE_TRAINING_TIMEOUT = float(os.environ.get("ONLINE_TRAINING_TIMEOUT", 300.0))
    ...
    ESN_SOC_SPECTRAL_RADIUS = float(os.environ.get("ESN_SOC_SPECTRAL_RADIUS", 0.30))
    ESN_SOC_LEAK_RATE    = float(os.environ.get("ESN_SOC_LEAK_RATE", 0.25))
    ESN_SOC_INPUT_SCALING = float(os.environ.get("ESN_SOC_INPUT_SCALING", 0.2))
    ...
    ESN_SOH_SPECTRAL_RADIUS = float(os.environ.get("ESN_SOH_SPECTRAL_RADIUS", 0.95))
    ESN_SOH_LEAK_RATE    = float(os.environ.get("ESN_SOH_LEAK_RATE", 0.10))
    ESN_SOH_INPUT_SCALING = float(os.environ.get("ESN_SOH_INPUT_SCALING", 0.8))
